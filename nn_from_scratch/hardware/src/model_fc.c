...
/* Fully connected model functionality, does forward propagation,
    ...
    @return nothing.
*/
void fc_calc_gradients(float *input, ModelPtr *model, float *actual, Gradients *gradients)
{
    float *curr_in = input;
    ...
    for (int i = 0; i < model->layers_size[model->n_layers - 1]; i++)
    {
        ...
        gradients->net_inputs[model->n_layers - 1][i] = loss_deriv * func(gradients->net_inputs[model->n_layers - 1][i], 1);
    }
    // perform backprop
...
void fc_apply_gradient(ModelPtr *model, int layer, int layer_size, int prev_layer_size, Gradients *gradients)
{
...
void fc_model_train(ModelPtr *model, float (*samples_x)[model->output_size], float (*samples_y)[model->output_size])
{
    ...
    {
        fc_calc_gradients(samples_x[BATCH_SIZE + i], model, samples_y[BATCH_SIZE + i], gradients);
    }
...
float *fc_model_predict(ModelPtr *model, float *input)
{
    ...
    int size = model->input_size;
    ActivationFunc func = getActivationFunc(model->layers_activation[0]);
    // forward propagate through each layer
    ...
    func = getActivationFunc(model->layers_activation[1]);
